...
while iter < MAX && diff1 > 10^-6
    ...
    % mat1 = kron(var.W,ones(2,1)).*var.E_est + var.tau*G;
    % mat2 = kron(var.W,ones(2,1)) + var.tau;
    A = mat1./mat2;

    % project A
    % M = sqrt(reshape(sum(reshape(A.^2,2,n^2),1),n,n));
    % A = (pi/4)*A./kron(M,[1;1]); % make all entries have norm pi/4
    ...
    % A_new = kron(Lambda,ones(2,1)).*A;
    % A_new = A_new*(norm(A,'fro')/norm(A_new,'fro'));
    % A = A_new;
    ...
    % var.lam = sign(var.lam).*min(5,abs(var.lam));
