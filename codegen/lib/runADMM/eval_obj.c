...
static void r_binary_expand_op(emxArray_real_T *in1, const emxArray_real_T *in2,
...
double eval_obj(double var_n, const emxArray_boolean_T *var_keep,
                ...
                const emxArray_real_T *var_lam, const emxArray_real_T *var_W,
                double *objLp)
{
  ...
  if ((var_E->size[0] == e->size[0]) && (var_E->size[1] == e->size[1]) &&
    ...
    r_binary_expand_op(e, var_E_est, var_E);
    e_data = e->data;
  }
  ma = var_W->size[0];
  na = var_W->size[1];
  ...
  if ((residual->size[0] == e->size[0]) && (residual->size[1] == e->size[1])) {
    ...
    combineVectorElements(b_residual, r);
    emxFree_real_T(&b_residual);
  } else {
    q_binary_expand_op(r, residual, e);
  }
  objL2 = 0.5 * b_sum(r);
  emxFree_real_T(&r);
  b_j1 = (int)var_n;
